...
// ✅ Ambil ID tertinggi dari tabel kategori untuk digunakan sebagai next ID
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['id']) && isset($_FILES['gambar'])) {
    ...
    $upload_dir = realpath(__DIR__ . '/../uploads') . '/';  // path fisik folder uploads
    ...
    if (move_uploaded_file($gambar_tmp, $target_file)) {
        $gambar_path_db = 'uploads/' . $filename; // path untuk disimpan di DB
    } else {
 ...
 <div class="modal-content">
  ...
  <form method="POST" enctype="multipart/form-data">
    ...
    <!-- <div class="form-group">
      ...
      <img id="preview_gambar_lama" src="" alt="Gambar Lama" width="120" style="border-radius: 8px;">
    </div> -->
...
function editModal(id, nama, keterangan, gambar) {
  console.log("EDIT DATA:", id, nama, keterangan, gambar); // debug
